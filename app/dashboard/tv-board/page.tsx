...
function clampLeadMinutes(n: number) {
...
async function saveCountdownLead(formData: FormData) {
  "use server";

  const raw = String(formData.get("countdown_lead_minutes") ?? "").trim();
  const n = clampLeadMinutes(Number(raw || DEFAULT_LEAD_MINUTES));
...
export default async function TvBoardDashboardPage() {
  ...
  // Load lineup countdown settings (server-side)
  const { data: lineupSettingsData, error: lineupSettingsErr } = await supabase
    .from("tv_lineup_settings")
    .select("id,countdown_lead_minutes,updated_at")
    .eq("id", "default")
    .maybeSingle();

  if (lineupSettingsErr) console.error("[tv-board] lineup settings error", lineupSettingsErr);
  ...
  // Schedules that are active "today" (ET day)
  const { data: scheduledTodayData, error: scheduledTodayErr } = await supabase
    .from("tv_sponsor_schedule")
    .select("id,sponsor_id,start_date,end_date,is_active,priority,created_at,sponsors(id,name,logo_path,tier,is_active)")
    .eq("is_active", true)
  ...
  return (
    <DashboardShell title="TV Board" subtitle={`Preview the TV + schedule sponsors (ET: ${todayEt})`} activeTab="dashboard">
      <div className="space-y-8">
        ...
        <section className="rounded-3xl border border-slate-100 bg-white px-4 py-6 shadow-sm sm:px-6 lg:px-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">This is the live TV board page. Use it to confirm layout + sponsor visibility.</p>
              {!key ? (
                <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                  <div className="font-semibold">CHECKIN_BOARD_KEY is missing on the server.</div>
                  <div className="mt-1 text-[13px]">
                    The TV page will show “TV Link Locked” until you set <code className="font-mono">CHECKIN_BOARD_KEY</code> in your
            ...
            <div className="mt-4 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex min-w-0 items-center gap-4">
                <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-white">
                  {currentLogoUrl ? (
                    <Image src={currentLogoUrl} alt={currentSponsor?.name || "Sponsor"} fill className="object-contain p-3" />
                  ) : (
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <div className="truncate text-base font-semibold text-slate-900">{currentSponsor?.name || "Unknown sponsor"}</div>
                    {currentSponsor?.tier ? (
                      <span className="rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-700">
                        {currentSponsor.tier}
                      </span>
                    ) : null}
                    <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${badgeClass(currentRow.is_active ? "emerald" : "slate")}`}>
        ...
        <section className="rounded-3xl border border-slate-100 bg-white px-4 py-6 shadow-sm sm:px-6 lg:px-8">
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white px-4 py-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="text-sm font-semibold text-slate-900">Countdown start window</div>
                <div className="mt-1 text-[12px] text-slate-600">
                  Before the <span className="font-semibold">first</span> set begins, hide the countdown until you’re within this many minutes.
                  During showtime, the countdown runs normally between sets.
                </div>
              </div>
              <div className="text-[11px] text-slate-500">
                Stored in <code className="font-mono">tv_lineup_settings</code>
              </div>
            </div>

            <form action={saveCountdownLead} className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div className="max-w-sm">
                <label className="text-xs font-semibold text-slate-700">Minutes before first set</label>
                <div className="mt-1 flex items-center gap-2">
                  <input
                    name="countdown_lead_minutes"
                    type="number"
                    min={0}
                    max={1440}
                    defaultValue={countdownLeadMinutes}
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-black"
              >
                Save countdown window
              </button>
            </form>

            <div className="mt-2 text-[11px] text-slate-500">
              Current setting: <span className="font-semibold text-slate-800">{countdownLeadMinutes} minutes</span>
            </div>
          </div>
        </section>
                ...
                return (
                  ...
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          <div className="truncate text-sm font-semibold text-slate-900">{s?.name || "Unknown sponsor"}</div>
                          {s?.tier ? (
                            <span className="rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-700">
                              {s.tier}
                            </span>
                    ...
                    <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-semibold ${badgeClass(row.is_active ? "emerald" : "slate")}`}>
        ...
        <section className="rounded-3xl border border-slate-100 bg-white px-4 py-6 shadow-sm sm:px-6 lg:px-8">
          ...
          <p className="mt-1 text-sm text-slate-500">This controls which sponsor(s) appear on the TV board — it does not change your main Sponsors list.</p>
        ...
        <section className="rounded-3xl border border-slate-100 bg-white px-4 py-6 shadow-sm sm:px-6 lg:px-8">
          ...
          {allSchedules.length === 0 ? (
            <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-600">No schedules yet.</div>
          ) : (
              ...
              <div className="min-w-[980px] bg-white">
                ...
                <div className="divide-y divide-slate-100">
                  {allSchedules.map((row) => {
                    const s = coerceSponsor((row as any).sponsors);
                    return (
                      <div key={row.id} className="grid grid-cols-[2.2fr_1fr_1fr_0.8fr_0.9fr_0.9fr] items-center gap-3 px-4 py-3 text-sm">
                        <div className="min-w-0">
                        ...
                        <form action={updateSchedule} className="contents">
                          ...
                          <div className="flex justify-center">
                            <input name="is_active" type="checkbox" defaultChecked={!!row.is_active} className="h-4 w-4" />
                          </div>
