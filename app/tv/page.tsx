...
// - Use service role client to avoid auth/RLS cookie issues
// - ET-aligned "today" using rewards_scans.scan_date + guest_checkins.day_et
// - ✅ Build-safe: recent[] is constructed ONLY from rows that have a real string atIso
...
export default async function TvPage(props: { searchParams?: Promise<{ key?: string }> }) {
  ...
  // Build VIP recent ONLY when scanned_at is a real string
  const vipRecent: RecentItem[] = (vipRows ?? [])
    .map((r: any) => {
      const atIso = r?.scanned_at;
  ...
  // Build Guest recent ONLY when scanned_at/checked_in_at is a real string
  const guestRecent: RecentItem[] = (guestRows ?? [])
    .map((r: any) => {
      const atIso = r?.scanned_at ?? r?.checked_in_at;
      if (!isNonEmptyString(atIso)) return null;
      return { atIso, label: "Guest" as const } satisfies RecentItem;
    })
    .filter((x): x is RecentItem => x !== null);

  // ✅ At this point: recent array has atIso: string ALWAYS. No filter needed.
      ...
      <div className="mx-auto max-w-6xl">
        <div className="flex items-end justify-between gap-6">
          <div className="min-w-0">
            <p className="text-[12px] uppercase tracking-[0.28em] text-slate-300">Sugarshack Downtown</p>
            <h1 className="mt-2 text-4xl md:text-6xl font-extrabold truncate">Check-Ins Today</h1>
            <p className="mt-2 text-slate-300">Live count · Updates every 5 seconds · Timezone: {ET_TZ}</p>
            <p className="mt-1 text-[12px] text-slate-400">
              Date (ET): <span className="font-semibold text-slate-200">{todayEt}</span>
            </p>
          </div>

          <div className="text-right">
            <p className="text-[12px] uppercase tracking-[0.28em] text-slate-400">Total</p>
            <p className="text-6xl md:text-8xl font-extrabold tabular-nums">{total}</p>
          </div>
        </div>

        <div className="mt-10 grid gap-6 md:grid-cols-2">
          <div className="rounded-3xl border border-slate-800 bg-slate-900/50 p-8">
            <p className="text-[12px] uppercase tracking-[0.22em] text-slate-300">VIP Check-Ins</p>
            <p className="mt-3 text-5xl font-bold tabular-nums">{vipCount}</p>
            <p className="mt-2 text-slate-400 text-sm">Rewards check-ins (source: qr_checkin)</p>
          </div>

          <div className="rounded-3xl border border-slate-800 bg-slate-900/50 p-8">
            <p className="text-[12px] uppercase tracking-[0.22em] text-slate-300">Guest Check-Ins</p>
            <p className="mt-3 text-5xl font-bold tabular-nums">{guestCount}</p>
            <p className="mt-2 text-slate-400 text-sm">Guest check-ins (no phone required)</p>
          </div>
        </div>

        <div className="mt-10 grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
          <div className="rounded-3xl border border-slate-800 bg-slate-900/40 p-8">
            <p className="text-xl md:text-2xl font-semibold">Scan the QR to check in & unlock rewards 🎉</p>
            <p className="mt-2 text-slate-300">VIPs earn points. Guests can still check in — then upgrade anytime.</p>
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-900/40 p-6">
            <p className="text-[12px] uppercase tracking-[0.22em] text-slate-300">Recent activity</p>
              ...
              <ul className="mt-3 space-y-2">
                {recent.slice(0, 10).map((r, idx) => (
                  <li key={idx} className="flex items-center justify-between gap-3">
                    <span className="text-sm font-semibold">{r.label === "VIP" ? "VIP" : "Guest"}</span>
